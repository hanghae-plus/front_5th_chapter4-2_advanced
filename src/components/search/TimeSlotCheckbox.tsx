...
const TimeSlotCheckbox = memo(({ timeSlot, isChecked, onToggle, idPrefix }: TimeSlotCheckboxProps) => {
  ...
  return (
    <Box key={timeSlot.id}>
      <Checkbox id={`${idPrefix}-timeslot-${timeSlot.id}`} size="sm" isChecked={isChecked} onChange={handleChange}>
