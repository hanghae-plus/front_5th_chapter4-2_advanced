...
const createAPICache = () => {
    ...
    return {
        fetchData: async (
            key: string,
            fetcher: () => Promise<AxiosResponse<Lecture[]>>
        ) => {
            if (!cache[key]) {
...
const fetchAllLectures = () => {
    ...
    return Promise.all([
        apiCache.fetchData("majors", fetchMajors),
        apiCache.fetchData("liberalArts", fetchLiberalArts),
    ]);
};
    ...
    useEffect(() => {
        ...
        fetchAllLectures().then((results) => {
            ...
            const uniqueLectures = Array.from(
                new Set(results.flatMap((result) => result.data))
            );
