...
const createCachedFetcher = () => {
  ...
  const cachedFetch = async (id: number, key: string, fetch: () => void) => {
    console.log(`API Call ${id}`, performance.now());
...
const cachedFatch = createCachedFetcher();

const fetchAllLectures = async () => {
  const result = await Promise.all([
    cachedFatch(1, "majors", fetchMajors),
    cachedFatch(2, "liberalArts", fetchLiberalArts),
  ]);
